...
void PageAirHum::initBackground()
{
...
PageAirHum::PageAirHum(sf::RenderWindow* window, sf::Event* ev, std::deque<Page*>* pages, std::vector<std::string>* dbLocation) : Page(window, ev, pages, dbLocation)
{
	if (!this->pages->empty())
...
void PageAirHum::endState()
{
	this->quit = true;
}

void PageAirHum::loadDatabaseData()
{
	std::vector<std::pair<std::string, std::string>> data;
	// if db->getCount() == -1, db was not found
	...
	{
		// Get last DATA_COUNT points from db
		for (int i = db->getCount(dbLocation->at(0).c_str(), dbLocation->at(1).c_str()) - this->DATA_COUNT; 
			i <= db->getCount(dbLocation->at(0).c_str(), dbLocation->at(1).c_str()); 
			i++)
		{
			std::string temp = "ID=";
...
void PageAirHum::updateData()
{
	...
	try
	{
		if (this->time.asSeconds() >= std::stof(temp))
		{
			...
			{
				this->graphs["GRAPH"]->updatePoints(std::stof(data[4].second));
				this->lastIndex++;
				this->clock.restart();
			}
		}
...
void PageAirHum::renderGUI(sf::RenderTarget * target)
{
...
void PageAirHum::render(sf::RenderTarget * target)
{
	if (!target)
		target = this->window;
	target->clear();
